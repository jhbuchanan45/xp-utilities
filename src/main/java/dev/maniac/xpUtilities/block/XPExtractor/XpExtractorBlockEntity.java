...
public class XpExtractorBlockEntity extends BlockEntity implements Storage<FluidVariant>, StorageView<FluidVariant>, BlockEntityClientSerializable {
    final FluidVariant fluid = FluidVariant.of(STILL_LIQUID_XP);
    final long capacity;
    long amount;
    private long version = 0;
    private final XpExtractorParticipant participant = new XpExtractorParticipant();

    public XpExtractorBlockEntity(BlockPos pos, BlockState state, long capacity) {
        super(XPBlockEntityTypes.XP_EXTRACTOR_BLOCK_ENTITY_TYPE, pos, state);
    ...
    private class XpExtractorParticipant extends SnapshotParticipant<ResourceAmount<FluidVariant>> {
        @Override
